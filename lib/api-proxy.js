...
function detectPlatform(videoUrl) {
  ...
  if (/tiktok\.com|vm\.tiktok|vt\.tiktok|tiktokv\.com/.test(u)) return 'tiktok';
  if (/instagram\.com|instagr\.am/.test(u)) return 'instagram';
  if (/facebook\.com|fb\.watch|fb\.com|fbcdn\.net/.test(u)) return 'facebook';
  if (/youtube\.com|youtu\.be|googlevideo\.com/.test(u)) return 'youtube';
  return 'other';
}

function getBackendConfigForPlatform(platform) {
  const cfg = loadApiConfig();
  const plat = String(platform || 'other').toLowerCase();
  const envKey = 'OMNI_BACKEND_' + plat.toUpperCase();
  const backends = cfg.backends && typeof cfg.backends === 'object' ? cfg.backends : {};
  const url = String(
    process.env[envKey] ||
    backends[plat] ||
    process.env.OMNI_BACKEND_URL ||
    cfg.backend_url ||
    ''
  ).trim().replace(/\/$/, '');
  const token = String(process.env.OMNI_API_TOKEN || cfg.api_token || '').trim();
  return { url, token, platform: plat };
}

function getBackendConfig(videoUrl) {
  if (videoUrl) return getBackendConfigForPlatform(detectPlatform(videoUrl));
  return getBackendConfigForPlatform('other');
}
...
const PLATFORM_UA = {
  tiktok: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36',
  instagram: 'Instagram 312.0.0.0.0 Android (33/13; 420dpi; 1080x2400; samsung; SM-G991B; o1s; exynos2100; en_US; 523000000)',
  facebook: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36',
  youtube: 'com.google.android.youtube/19.45.36 (Linux; U; Android 12; en_US) gzip',
};

function upstreamHeaders(mediaUrl, pageUrl) {
  const page = String(pageUrl || mediaUrl || '');
  const platform = detectPlatform(page);
  const ua = PLATFORM_UA[platform] || PLATFORM_UA.tiktok;

  if (/googlevideo\.com|gvt1\.com/i.test(String(mediaUrl || '')) || platform === 'youtube') {
    return {
      Referer: 'https://www.youtube.com/',
      Origin: 'https://www.youtube.com',
      Accept: '*/*',
      'Accept-Language': 'en-US,en;q=0.9',
      'User-Agent': PLATFORM_UA.youtube,
      Range: 'bytes=0-',
    };
  }
  try {
    const host = new URL(String(mediaUrl || '')).hostname.toLowerCase();
    if (/tiktok|tiktokcdn|ttwstatic|muscdn|byteoversea|tiktokv/.test(host) || platform === 'tiktok') {
      return {
        Referer: 'https://www.tiktok.com/',
        Origin: 'https://www.tiktok.com',
        Accept: 'video/mp4,video/*,*/*;q=0.8',
        'Accept-Language': 'en-US,en;q=0.9',
        'User-Agent': PLATFORM_UA.tiktok,
        Range: 'bytes=0-',
      };
    }
    if (/instagram|cdninstagram|fbcdn.*instagram/i.test(host) || platform === 'instagram') {
      return {
        Referer: 'https://www.instagram.com/',
        Origin: 'https://www.instagram.com',
        Accept: 'video/mp4,video/*,image/*,*/*;q=0.8',
        'Accept-Language': 'en-US,en;q=0.9',
        'User-Agent': PLATFORM_UA.instagram,
        Range: 'bytes=0-',
      };
    }
    if (/fbcdn|facebook|fb\.watch/i.test(host) || platform === 'facebook') {
      return {
        Referer: 'https://www.facebook.com/',
        Origin: 'https://www.facebook.com',
        Accept: 'video/mp4,video/*,*/*;q=0.8',
        'Accept-Language': 'en-US,en;q=0.9',
        'User-Agent': PLATFORM_UA.facebook,
        Range: 'bytes=0-',
      };
    }
  } catch (_) { /* ignore */ }
...
async function upstreamExternalBackend(videoUrl) {
  ...
  const { url, token } = getBackendConfigForPlatform(platform);
  if (!url) return null;
  ...
  try {
    ...
    const response = await fetch(url + '/api/download', {
      method: 'POST',
      headers: headers,
      body: JSON.stringify({ url: videoUrl, platform: platform }),
...
async function proxyDownload(videoUrl, retries) {
  const maxRetries = retries != null ? retries : 1;
  let lastStatus = 502;
...
async function fetchExternalStream(params) {
  const pageUrl = (params && (params.page_url || params.url)) || '';
  ...
  if (!qs.get('platform')) qs.set('platform', platform);
