...
function isRapidApiQuotaError(status, data) {
  const msg = String((data && (data.message || data.error)) || '').toLowerCase();
  return status === 429 || /quota|exceeded|monthly limit|upgrade your plan|too many requests/i.test(msg);
}
...
async function upstreamDownload(videoUrl) {
  if (hasExternalBackend()) {
    ...
    return {
      status: 502,
      data: {
        error: 'External backend unavailable',
        message: 'yt-dlp backend did not respond — try again in a moment.',
      },
      rateLimit: null,
    };
  }
  ...
  try {
    ...
    if (isRapidApiQuotaError(response.status, data) && hasExternalBackend()) {
      const external = await upstreamExternalBackend(videoUrl);
      if (external && external.status === 200 && external.data && external.data.error !== true) {
        return external;
      }
    }
    ...
    try {
      ...
      if (/quota|exceeded|monthly limit|upgrade your plan|too many requests/i.test(msg)) {
        if (hasExternalBackend()) {
          const external = await upstreamExternalBackend(videoUrl);
          if (external && external.status === 200 && external.data && external.data.error !== true) {
            return { status: external.status, data: external.data, rateLimit: lastRateLimit };
          }
        }
        return { status, data, rateLimit: lastRateLimit };
      }
