...
const MAX_CONCURRENT = Math.max(1, parseInt(process.env.YT_DLP_CONCURRENCY || '20', 10));
const MAX_WAITING = Math.max(0, parseInt(process.env.YT_DLP_QUEUE_SIZE || '200', 10));
const QUEUE_TIMEOUT_MS = Math.max(30000, parseInt(process.env.YT_DLP_QUEUE_TIMEOUT_MS || '180000', 10));
...
async function withRedisDownloadSlot(fn) {
  ...
  try {
      ...
      if (active <= MAX_CONCURRENT) {
        try {
          ...
          try {
            ...
            console.warn('[download-queue] redis decr failed:', decrErr.message);
          }
        }
      }
    ...
    if (/wrongpass|invalid username-password|redis unavailable|disabled/i.test(String(err.message || ''))) {
      console.warn('[download-queue] Redis slot error, using local queue:', err.message);
