...
async function updateDownloadApiFromLiveQuota() {
  return null;
}
...
async function syncDownloadApiUsageFromEvents() {
  ...
  const now = Date.now();
  const planKey = 'download-api';

  if (usingRedis) {
    const raw = await redisExec(['HGET', REDIS_PLANS_KEY, planKey]);
    if (raw && raw.result != null) {
      ...
      await redisExec(['HSET', REDIS_PLANS_KEY, planKey, JSON.stringify(plan)]);
    }
    return eventTotal;
  }
  ...
  if (usingBlobs) {
    const map = await blobGetPlansMap();
    if (map[planKey]) {
      map[planKey].used = mergeRapidUsageCounts(map[planKey].used, eventTotal, null, baseline);
      map[planKey].updated_at = now;
      map[planKey].meta = Object.assign({}, map[planKey].meta || {}, { synced_from_events: true, api_ok: apiOk, api_fail: apiFail });
      await blobSavePlansMap(map);
    }
    return eventTotal;
  }

  const existing = await getPlan(planKey);
...
function buildFallbackDashboard(storageError, netlifyLive) {
  const plans = defaultPlans().map(function (plan) {
    ...
    if (plan.provider === 'omni-ytdlp') {
      enrichDownloadApiPlan(copy, 0);
    }
  ...
  const hasDownloadApi = plans.some(function (p) { return p.provider === 'omni-ytdlp'; });

  return {
    live: hasDownloadApi || Boolean(netlifyLive),
...
function enrichDownloadApiPlan(plan, eventUsed) {
  const baseline = getRapidApiBaseline();
  plan.used = mergeRapidUsageCounts(plan.used, eventUsed, null, baseline);
  plan.percent = planPercent(plan);
  plan.live = false;
  plan.badge_label = null;
  plan.meta = Object.assign({}, plan.meta || {}, {
    source: 'Tracked from admin download API events',
  });
...
async function getDashboardStats() {
  ...
  if (storageError) {
    return buildFallbackDashboard(storageError, netlifyLiveEarly);
  }

  try {
    await syncPlanLimitsFromTier();
    await syncDownloadApiUsageFromEvents();
  } catch (err) {
  ...
  try {
    ...
    const fallback = buildFallbackDashboard(err.message, netlifyLiveEarly);
  ...
  const enrichedPlans = plans.map(function (plan) {
    ...
    if (plan.provider === 'domain') enrichDomainPlan(copy);
    if (plan.provider === 'omni-ytdlp') {
      enrichDownloadApiPlan(copy, copy.used);
    }
...
module.exports = {
  ...
  incrementPlanUsage,
  updatePlanLimit,
  updateDownloadApiFromLiveQuota,
  getDashboardStats,
  getRecentEvents,
  getAllPlans,
};
