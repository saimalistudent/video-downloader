...
async function fetchMetadataDirect(videoUrl, options) {
  options = options || {};
  const refresh = options.refresh === true;
  const urlKey = String(videoUrl || '').trim();
  if (!urlKey) {
    return { immediate: true, status: 400, data: { error: 'Missing url' }, fromCache: false };
  }
  ...
  if (inflightLookups.has(urlKey)) {
    return inflightLookups.get(urlKey);
  }

  const pending = (async function () {
    const { proxyDownload } = require('./api-proxy');
    const { status, data } = await proxyDownload(urlKey, 1);
    if (isSuccessfulApiPayload(status, data)) {
      await setCachedLink(urlKey, status, data);
    }
    return {
      immediate: true,
      status: status,
  ...
  inflightLookups.set(urlKey, pending);
  try {
    return await pending;
  } finally {
    inflightLookups.delete(urlKey);
  }
}

async function resolveDownloadWithCache(videoUrl, options) {
  const result = await fetchMetadataDirect(videoUrl, options || {});
  if (result.immediate) {
    return {
